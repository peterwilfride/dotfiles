...
directory="/usr/share/icons/Papirus/48x48/apps"
...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme ~/.config/rofi/rofi-notification/config.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "Notifications" -no-fixed-num-lines -lines 8 -i -no-config)
...
if [ -n "$selection" ]; then
    ...
    #echo "$remove_markup"
    extract_date="${remove_markup:20:11}"
    #echo "$extract_date"
    remove_whitespace=$(echo -ne "$remove_markup" | sed '1~3 s/\s.*$//')
    remove_whitespace=$(echo -ne "$remove_whitespace" | sed '$d')
    #echo "$remove_whitespace"
    add_timestamp="$remove_whitespace\n.*\n.*\n$extract_date\n"
    #add_timestamp="$extract_date\n.*\n.*\n$remove_whitespace\n"
    #echo -ne "$add_timestamp"
    reverse_search=$(tac <(echo -ne "$add_timestamp") | sed -E ':a;N;$!ba;s/\r{0,1}\n/\\n/g')
    echo -ne "$reverse_search"
    echo ""
    echo ""
    sed -i "N;N;N;N;N; /^$reverse_search$/d" /tmp/dunstlog
    #sed "/^$reverse_search$/d" /tmp/dunstlog
fi
